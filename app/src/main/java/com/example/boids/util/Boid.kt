...
class Boid {
    ...
    fun flock(boids: Array<Boid?>, neighbours: IntArray, distances: FloatArray): Vector{
        var separation = separate(boids,neighbours,distances).multiply(SEPARATION_WEIGHT)
        val alignment = align(boids,neighbours).multiply(ALIGNMENT_WEIGHT)
        val cohesion = cohere(boids,neighbours).multiply(COHESION_WEIGHT)
